...
mapview(res)
```
L'API comprend des limites : l'emprise de la surface d'interrogation et le nombre de requêtes envoyées par jour sont limités.
...
Voici un [exemple d'usage d'`{osmextract}`](https://gitlab-forge.din.developpement-durable.gouv.fr/dreal-pdl/csd/sgbd_datamart/-/blob/master/data-raw/chargement_pistes_cyclables_osm.R), toujours sur les pistes cyclables, qui va de la récupération des données au calcul d'indicateurs par commune. 
...
On procède en 2 temps en raison de la limite d'interrogation :  

```{r osrmtable2, eval=TRUE}
# distances avec les 400 premiers restaurants

# distances avec les 250 premiers restaurants
mat1 <- osrmTable(src = mes_adresses,                  # dans le résultat, on aura une ligne par point src
                  dst = restaurants_nantes[1:250, ],   # dans le résultat, on aura une colonne par point dst
                  osrm.profile = "foot")
# distances avec les 250 restaurants suivants
mat2 <- osrmTable(src = mes_adresses,                  # dans le résultat, on aura une ligne par point src
                  dst = restaurants_nantes[251:500, ],   # dans le résultat, on aura une colonne par point dst
                  osrm.profile = "foot")

# distances avec les restaurants suivants
mat3 <- osrmTable(src = mes_adresses,
                  dst = restaurants_nantes[501:nrow(restaurants_nantes), ],
                  osrm.profile = "foot")
# Assemblage
mat <- list(durations = bind_cols(mat1$durations, mat2$durations, mat3$durations, .name_repair = "unique_quiet"),
            sources = bind_rows(mat1$sources, mat2$sources, mat3$sources),
            destinations = bind_rows(mat1$destinations, mat2$destinations, mat3$destinations))
