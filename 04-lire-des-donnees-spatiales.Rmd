...
## Données adaministratives contenues dans des packages
...
## Géoparquet
Parquet est un format de données compact, puissant et efficace, dont l'usage se développe fortement. Il est particulièrement adapté aux données volumineuses.  

GeoParquet est une norme de format, en développement rapide, de l'Open Geospatial Consortium (OGC) qui ajoute des types géospatiaux interopérables (Point, Ligne, Polygone) à Parquet. 

Un fichier (geo)parquet peut contenir une ou plusieurs tables et inclut des métadonnées.  
On ne peut lire qu'une partie des données contenues dans le fichier, en formulant des requêtes, et ainsi accélérer les traitements. 

`{sf}` ne dispose pas (encore ?) de driver pour lire ou interroger des fichiers geoparquet. 
Il faut installer le package `{geoarrow}` pour cela

Voici un exemple de lecture et d'exploitation de données geoparquet :
```{r geoparquet, eval = FALSE}
library(arrow)
library(geoarrow)
library(sf)
library(dplyr)

# URL du fichier GeoParquet (Natural Earth Countries)
...
# Lire directement depuis l'URL (sans téléchargement intermédiaire)
countries_sf <- read_parquet(url, as_sf = TRUE)

# 
# duckdb::duckdb_register_arrow("extdata/countries.parquet")
# 
# dataset <- open_dataset("extdata/countries.parquet")
# metadata <- dataset$metadata
# 
# # Créer un point d'intérêt (ex: Paris)
# paris <- st_point(c(2.3522, 48.8566)) %>%  st_sfc(crs = 4326)
# 
# # Lire les pays autour de Paris (buffer de 500 km)
# around_paris_sf <- read_geoparquet("countries.parquet", 
#                                    filter = list(geometry = list(intersects = list(geometry = st_buffer(paris, dist = 500000), crs = 4326))))
