...
# Créer des cartes avec ggplot2 {#creer-des-cartes-avec-ggplot2}
Dans ce chapitre, nous allons utiliser les librairies suivantes.

```{r pkg_chap_12_ggplot}
library(sf)
library(tmap)
library(tidyverse)
library(ggplot2)
...
`{ggplot2}` est un package de visualisation de données qui dispose de templates (thèmes, palettes) à la Marque Etat grâce au package `{gouvdown}`, créé par la communauté d'agents de l'Etat [Spyrales](https://www.spyrales.fr/).
...
Chaque fonction s’enchaîne avec des `+` comme les `%>%`.
...
<!-- ### Le thème et les palettes conformes à la marque Etat -->
<!-- TODO -->
...
Chaque nouvel élément graphique est à rajouter sous forme de layer, ici nous utilisons la fonction `labs()` qui permet de labelliser tout les éléments possibles de l'aesthetic, ainsi que le titre (`title`), le sous titre (`subtitle`) et le bas de page (`caption`).
...
![](pic/palette.PNG){#id .class width=500}
...
ggplot(sdg_indicators) +
  geom_point(aes(x = gdp_per_cap, y = sh_sta_mmr, color = continent)) +
...
Les fonctions `scales()` permettent globalement de paramétrer les éléments rentrés dans l'aesthetic : 
...
- le nombre de colonnes ou de ligne sur lesquels on veut voir s'afficher le graphique
...
ggplot2 intègre une fonction geom permettant l'utilisation de données géographiques : `geom_sf()`.

Celle-ci doit se coupler avec la fonction `coord_sf()` qui permet de s'assurer en cas de superposition de couches que celles-ci utiliseront bien le même crs en spécifiant le datum.  
`coord_sf()` permet également de *zoomer* sur la carte en spécifiant les bornes x et y de la carte, ou également de définir les labels des axes.
...
```{r, echo=TRUE, eval=TRUE, warning=FALSE, fig.height=5, fig.width=12}
...
On peut exploiter de la même façon les différentes fonctions vues précédemment. Par exemple avec un peu de thème et de facettes.
...
sdg_indicators_sf %>% 
  filter(timeperiod %in% c("2000", "2005", "2010", "2015")) %>% 
           ...
           facet_wrap(~timeperiod, drop = TRUE)
...
sdg_indicators_sf %>% 
  filter(timeperiod == "2015", !is.na(sh_sta_mmr)) %>% 
    ggplot() + 
    geom_sf(fill = "white") +
    stat_sf_coordinates(aes(color = sh_sta_mmr, size = sh_sta_mmr)) +
    theme_gouv_map() +
    theme(panel.background = element_rect(fill = "light blue"))
