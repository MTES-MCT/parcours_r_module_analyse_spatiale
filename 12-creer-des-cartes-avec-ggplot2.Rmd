...
`{ggplot2}` est un package du tidyverse, qui propose une syntaxe cohérente entre dataviz et cartoviz. Il dispose des templates à la Marque Etat, raison pour laquelle son utilisation est à privilégier au sein du pôle ministériel.  
...
Comme l'utilisation de `{ggplot2}` est à privilégier, ce package ne sera pas présenté lors de la formation.
...
```
On peut modifier ici le parametre `stat` de geom_bar pour afficher la moyenne des longueurs de sépales par espèces :
...
- `geom_sf()` : pour visualiser nos objets géographiques  
...
ggreg + geom_sf_text(aes(label = REG)) + labs(title = "geom_sf_text") + 
  ggreg + geom_sf_label(aes(label = REG)) + labs(title = "geom_sf_label") 
 
```

Les fonctions `geom_sf_text()` et `geom_sf_label()` sont associées à la fonction de transformation `stat_sf_coordinates()` qui par défaut renvoie un point au centre de nos objets géographiques pour placer l'annotation textuelle (fonction `sf::st_point_on_surface()`).
...
Comme avec d'autres packages, la première étape consiste à préparer le jeu de données à visualiser, c'est à dire d'associer les variables à analyser à leur géométrie. 
...
carte_ronds_prop2 +
  coord_sf(datum = 2154) +
  annotate("text", x = 400000, y = 6800000, label = "j'ajoute mon texte", color = "blue")
```
...
Le formatage des valeurs des axes ou de la légende se fait à l'aide d'une fonction de formatage des nombres passée à l'argument 'labels' des fonctions scale
...
Il faut au préalable préparer notre jeu de données en [le passant au format long avec pivot_longer()](https://mtes-mct.github.io/parcours_r_socle_preparation_des_donnees/structurer-ses-tables.html#les-deux-fonctions-clefs-de-tidyr).
