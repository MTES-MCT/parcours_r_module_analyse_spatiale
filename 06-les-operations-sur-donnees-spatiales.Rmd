...
On aurait aussi pu écrire en *tidyverse* :
...
mapview(list(departement_44_buffer, departement_44), layer.name = c("Loire-Atlantique avec un buffer de 5 km", "Loire-Atlantique"), 
...
La fonction `st_make_grid()` permet de créer une grille  régulière (carroyage) à partir d'un objet géo. 
...
mapview(list(grille_ajustee, epci_d44))
```
...
mapview(list(grille_ajustee, epci_d44, boul_44))
```
...
L'objet **intersection** est une liste de la longueur de l'objet **grille_ajustee** et chaque élément de la liste contient l'index des éléments de l'objet **boul_44** qu'il intersecte.
...
Pour compter le nombre de boulangerie par carreau, on peut parcourir la liste et de reporter la longueur de chacun de ces éléments. 
...
```{r}
nb_boul_par_carreau <- st_join(x = grille_ajustee, y = boul_44, join = st_contains, left = FALSE) %>% 
  st_drop_geometry() %>% 
  group_by(id) %>% 
  summarise(nb_boulang = n())

grille_boul2 <- grille_ajustee %>% 
  left_join(nb_boul_par_carreau, by = "id")

select(grille_boul2, nb_boulang) %>% plot
```
