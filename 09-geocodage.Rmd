...
Le package `{tidygeocoder}` utilise un grand nombre de services de géocodage en ligne et est le plus universel. 
...
Le résultat est un df enrichi de 15 nouveaux attributs dont le nom commence par `result_` + un champ longitude et un champ latitude. 
...
Il reste alors à transformer le résultat en df spatial, grâce à la fonction `st_as_sf()` vue au chapitre \@ref(lire-des-donnees-spatiales).
