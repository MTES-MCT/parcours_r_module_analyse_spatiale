...
library(melodi)     # remotes::install_github("InseeFrLab/melodi")
...
### Préparation des données {#melodi} 
...
Ici, nous partons des données de population les plus récentes, diffusées par l'INSEE par API et mobilisable dans R avec le package `{melodi}`, et des fonds de carte disponibles dans le package `{COGiter}`.
...
# téléchargement API du dernier millésime de la population de référence
pop_preparee_nat <- get_all_data("DS_POPULATIONS_REFERENCE") %>% 
  filter(TIME_PERIOD == max(TIME_PERIOD), GEO_OBJECT == "COM") %>% 
  mutate(TIME_PERIOD = as.character(TIME_PERIOD)) %>% 
  select(-GEO_OBJECT, -FREQ ) %>% 
  # mise à jour du COG et agrégation aux différentes échelles adm
  cogifier(code_commune = GEO, communes = TRUE, epci = TRUE, departements = TRUE, regions = TRUE, metro = FALSE)

# on filtre le jeu de données pour ne garder que les territoires de Normandie et toutes les régions
pop_preparee <- pop_preparee_nat %>% 
  filtrer_cog(reg = "28", garder_supra = ">=") %>% 
  # passage au format large
  pivot_wider(names_from = POPREF_MEASURE, values_from = OBS_VALUE)

# Aperçu du jeu de données
glimpse(pop_preparee)
...
pop_epci_normands_geo <- fonds_carto_normands$epci %>% 
  left_join(pop_preparee, by = c("EPCI" = "CodeZone")) %>% 
  # calcul des indicateurs a représenter
  mutate(poids_pop_a_part = PCAP / PTOT * 100,
         densite_pop = as.numeric(PMUN / AREA * 1000000))
```
...
ggplot(data = pop_epci_normands_geo) +
...
carte_ronds_prop <- ggplot(data = pop_epci_normands_geo) +
  geom_sf() +
  stat_sf_coordinates(mapping = aes(size = PTOT, color = poids_pop_a_part)) +
  theme_map()
...
carte_ronds_prop3 <- carte_ronds_prop2 +
  guides(color = guide_legend(direction = "horizontal", order = 1, nrow = 1, 
...
![](https://spyrales.github.io/gouvdown/reference/figures/README-palettes-1.png)  
...
### Exercice 3 - Carte des données dvf par quartier de Nantes
...
indic_epci_normands_geo_long <- pop_epci_normands_geo %>% 
  pivot_longer(cols = c(starts_with("P"), densite_pop, poids_pop_a_part), 
               names_to = "indicateurs", values_to = "valeurs") %>% 
  filter(str_detect(indicateurs, "^P"))
...
fonds_carto_empiles <- bind_rows(
  ...
  inner_join(pop_preparee, by = c("CodeZone", "TypeZone")) %>% 
  mutate(densite = round(PMUN / as.numeric(AREA) * 1000000, 0),
...
carte_ronds_prop3 +
  ...
  annotation_north_arrow(location = "bl", height = unit(0.5, "cm"), width = unit(0.5, "cm"))
```
...
Filtrons par exemple sur le Département du Calvados.
...
On peut ensuite réaliser une carte zoomée sur le Calvados
...
calvados <- normandie + 
  ...
  scale_color_gouv_continuous(palette = "pal_gouv_i", reverse = FALSE) +
  scale_size_continuous(labels = function(x) format(x, big.mark = " ", scientific = FALSE)) +
  labs(title = "Population par EPCI du Calvados",
       subtitle = paste0("En ", mil_pop), x = "", y = "",
       size = "Nombre d'habitants",
       color = "Poids population\ncomptée à part (%)",
       caption = "Source : INSEE - RP")
...
pop_prepa_nat <- pop_preparee_nat %>% 
  # passage au format large
  pivot_wider(names_from = POPREF_MEASURE, values_from = OBS_VALUE) %>% 
  mutate(poids_pop_a_part = PCAP / PTOT * 100)

creer_carte_communes_prop(data = pop_prepa_nat, code_region = "02", indicateur = PMUN,
                          carto = fond_carto("Martinique", ombre = 0), espace = " ",
                          palette = "pal_gouv_o", titre = "Population par commune", sous_titre = paste0("En ", mil_pop),
...
Toutes ces fonctions sont des cas particuliers de la fonction `creer_carte()` :

```{r, message=FALSE, warning=FALSE}
creer_carte(data = pop_prepa_nat, code_region = "28", indicateur = poids_pop_a_part, maillage = "Département",
            carto = fond_carto("Normandie"), titre = "Proportion de la population comptée à part", sous_titre = paste0("En ", mil_pop),
            bas_de_page = "INSEE - RP", decimales = 1, suffixe = " %", palette = "pal_gouv_b", espace = "\u202f" )
```
