...
```{r}
bins <- quantile(sdg_indicators_2015_sf$sh_sta_mmr, na.rm = TRUE)
```
...
```{r}
pal <- colorBin("YlOrRd", domain = sdg_indicators_2015_sf$sh_sta_mmr, bins = bins)
```
...
```{r}
bins <- quantile(sdg_indicators_2015_sf_centroid$sh_sta_mmr, na.rm = TRUE)
```
...
```{r}
labels <- sprintf("<strong>%s</strong><br/>%g décès pour 100 000 naissance en 2015", 
                  sdg_indicators_2015_sf_centroid$geoareaname, 
                  sdg_indicators_2015_sf_centroid$sh_sta_mmr) %>% 
  lapply(htmltools::HTML)
```
